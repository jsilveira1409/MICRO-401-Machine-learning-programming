function [X_train, Y_train, X_test, Y_test] = split_data(X, Y, validSize)
    %SPLIT_DATA Split a dataset between train and test data
    
    % Calculate the number of elements
    nbSamples = size(X, 2);
    
    % Shuffle both X and Y in the same way
    % HINT: look up the function randperm in the documentation and indexing
    % in the tips file
    ...
    for i=1:nbClasses
        ...
        % HINT: it corresponds to the ratio of elements of the i-th class
        % in the whole dataset. Consider using the function sum over a
        % logical array
    ...
    % Initialize an array to keep track of the current number of samples
    % per class
    ...
    for i=1:nbSamples
        ...
        % Add the element in the train set if you have not reached the
        % desired number of train sample for this class
        % HINT: consider performing a simple if else test here. Start with
        % the train set.
